var app = (function () {
    ...
    /* src\Space.svelte generated by Svelte v3.22.2 */
    ...
    function create_fragment(ctx) {
    	...
    	return {
    		c() {
    			div = element("div");
    			t = text(/*space*/ ctx[0]);
    			attr(div, "class", "space svelte-gg98m3");
        ...
        try {
            if (!connection || connection.readyState >= 2) {
                ...
                connection.onclose = (e) => {
                    ...
                    connectionTimer = setInterval(() => connect(), 500);
                };
    ...
    const nextMove = async (space) => {
        try {
            const response = await fetch(
                `${{"env":{"apiUrl":"https://tictactoe.noahglaser.net","wsUrl":"wss://tictactoe.noahglaser.net/ws"}}.env.apiUrl}/next-turn/${space}`
            );
    ...
    function create_if_block_3(ctx) {
    	...
    	return {
    		...
    		p(ctx, dirty) {
    			if (dirty & /*winner*/ 4) set_data(t0, /*winner*/ ctx[2]);
    		},
    		d(detaching) {
    			if (detaching) detach(h2);
    		}
